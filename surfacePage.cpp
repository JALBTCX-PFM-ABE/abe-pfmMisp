  ...
  QWizardPage (parent)
{
  ...
  QGroupBox *nBox = new QGroupBox (tr ("Clear interpolated data"), this);
  QHBoxLayout *nBoxLayout = new QHBoxLayout;
  nBox->setLayout (nBoxLayout);
  nBoxLayout->setSpacing (10);

  nFlag = new QCheckBox (this);
  nFlag->setToolTip (tr ("Set to clear interpolated data"));
  nFlag->setWhatsThis (nFlagText);
  nFlag->setChecked (options->clear_int);
  nBoxLayout->addWidget (nFlag);
  ...
  nibble->setToolTip (tr ("Set the cell distance from real data"));
